...
function assertAllowedStdioCommand(command: string): void {
  ...
  const allowed = new Set(
    (process.env["MCP_LAB_ALLOWED_COMMANDS"] ?? "node,npx,npm,pnpm")
      .split(",")
  ...
  if (!commandName || !allowed.has(commandName)) {
    throw new Error(`Stdio command "${command}" is not allowed by the MCP Lab command policy`);
  }
}
