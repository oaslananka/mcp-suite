  ...
  return (
    <div style={styles.page}>
      <section style={styles.leftPanel}>
        ...
        <div style={styles.toolList}>
          ...
          {filteredTools.length === 0 ? (
            <div style={styles.empty}>No tools match this search.</div>
          ) : null}
        </div>
      </section>
      ...
      <section style={styles.rightPanel}>
        <div style={styles.panelHeader}>
          <div>
            ...
            <div style={styles.subtitle}>
              {selectedTool?.description ?? "Connect to a server to inspect tool contracts."}
            </div>
          </div>
          <div style={styles.latency}>
            {latency ? `${latency} ms` : status === "connected" ? "Ready" : "Disconnected"}
          </div>
        </div>

        <JsonEditor label="Arguments" value={requestJson} onChange={setRequestJson} height={220} />
        ...
        <JsonEditor label="Response" value={responseJson} readOnly height={260} />
      </section>
    </div>
  );
}
