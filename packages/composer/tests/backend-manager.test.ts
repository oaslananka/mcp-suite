...
vi.mock("@oaslananka/shared", () => ({
  ...
  StdioTransport: class {
    constructor(
      public readonly stdout: unknown,
      public readonly stdin: unknown
    ) {
...
describe("BackendManager", () => {
  ...
  it("connects stdio backends, validates required fields, and can reconnect", async () => {
    ...
    await expect(manager.addBackend("broken-http", { transport: "http" })).rejects.toThrow(
      "missing url"
    );
    await expect(manager.addBackend("broken-stdio", { transport: "stdio" })).rejects.toThrow(
      "missing command"
    );
