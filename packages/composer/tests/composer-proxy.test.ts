...
describe("ComposerProxy", () => {
  it("lists namespaced tools, routes tool calls, and aggregates resources and prompts", async () => {
    ...
    expect(
      await (proxy as any).callTool({ name: "github__search", arguments: { q: "mcp" } })
    ).toMatchObject({
    ...
    await expect((proxy as any).callTool({ name: "missing__search" })).rejects.toThrow(
      'Backend "missing" is not connected'
    );
  });
});
