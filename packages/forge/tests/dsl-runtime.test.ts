...
`)
  ...
  it("evaluates conditions and templates with helper functions and graceful failures", () => {
    ...
    expect(
      transformer.transform("Hello {{ uppercase(user.name) }}", { user: { name: "ada" } })
    ).toBe("Hello ADA");
    expect(
      transformer.transform("{{ labels | includes('bug') }}", { labels: ["bug", "triage"] })
    ).toBe(true);
  ...
  it("hydrates run context with vars and resolves server config lookups", () => {
    const ctx = new RunContext("run-1", "pipeline-1", { region: "eu-west-1" }, undefined, {
      api: { transport: "http", url: "https://api.example.com" },
    });

    expect(ctx.dataBus.get("region")).toBe("eu-west-1");
    expect(ctx.getServerConfig("api")).toEqual({
      transport: "http",
  ...
  it("reuses pooled clients, cleans up idle connections, and shuts down the pool", async () => {
    ...
    const entry = ((
      pool as unknown as { pools: Map<string, Array<{ lastUsed: number }>> }
    ).pools.get("sentinel") ?? [])[0];
    if (!entry) {
