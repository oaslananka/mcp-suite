...
vi.mock("@oaslananka/shared", () => {
  const MCPClient = vi.fn().mockImplementation(function MockMCPClient(
    transport: unknown,
    options: unknown
  ) {
    return {
  ...
  const StreamableHTTPTransport = vi.fn().mockImplementation(function MockStreamableHTTPTransport(
    options: unknown
  ) {
    return { kind: "http", options };
  });

  const StdioTransport = vi.fn().mockImplementation(function MockStdioTransport(
    stdout: unknown,
    stdin: unknown
  ) {
