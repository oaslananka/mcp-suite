...
describe("Forge nodes", () => {
    ...
    const loopResult = await loopNode.execute(
      {
        ...
        over: "{{ items }}",
        as: "item",
        steps: [{ id: "echo", type: "log", message: "Hello {{ item.name }}" }],
      },
      ctx
    );
  ...
  it("resolves tool inputs recursively and retries transient tool failures", async () => {
    ...
    expect(ctx.connectionManager.getClient).toHaveBeenCalledWith("tools", {
      transport: "http",
  ...
  it("creates node implementations from step config and rejects unknown step types", () => {
    expect(createNode({ id: "tool", server: "tools", tool: "list", input: {} })).toBeInstanceOf(
      ToolCallNode
    );
    expect(
      createNode({
