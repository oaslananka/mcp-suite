...
export class RunStore {
  ...
  public savePipeline(config: PipelineConfig): PipelineConfig {
    const id = config.name;
    const existing = this.db.prepare(`SELECT id FROM pipelines WHERE id = ?`).get(id);
    if (existing) {
      this.db
        .prepare(
          `
        ...
        .run(config.name, JSON.stringify(config), id);
    } else {
      this.db
        .prepare(
          `
        ...
        .run(id, config.name, JSON.stringify(config));
    }

    const saved = this.getPipeline(id);
    if (!saved) {
      throw new Error(`Pipeline "${id}" was not saved`);
    }
    return saved;
  }
  ...
  public listPipelines(): PipelineConfig[] {
    const rows = this.db
      .prepare(`SELECT config_json FROM pipelines ORDER BY updated_at DESC, name ASC`)
      .all() as Array<{ config_json: string }>;
    return rows.map((row) => JSON.parse(row.config_json) as PipelineConfig);
  }

  public getPipeline(id: string): PipelineConfig | null {
    const row = this.db
      .prepare(`SELECT config_json FROM pipelines WHERE id = ? LIMIT 1`)
      .get(id) as { config_json: string } | undefined;
    return row ? (JSON.parse(row.config_json) as PipelineConfig) : null;
  }
