...
import { ApiServer, type ApiServerOptions } from "./server/ApiServer.js";
...
program.name("forge").description("Chain MCP tools into declarative pipelines").version("1.0.0");
...
program
  ...
  .action(async (options: { port: string }) => {
    const engine = new ForgeEngine({ dbPath: process.env["FORGE_DB_PATH"] || ":memory:" });
    const store = new RunStore(process.env["FORGE_DB_PATH"] || ":memory:");
    const apiOptions: ApiServerOptions = {
      allowedOrigins:
    ...
    const authToken = process.env["FORGE_API_TOKEN"];
    if (authToken) {
      apiOptions.authToken = authToken;
    }
    const server = new ApiServer(engine, store, apiOptions);
