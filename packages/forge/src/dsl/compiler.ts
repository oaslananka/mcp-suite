...
export function compile(config: PipelineConfig): PipelineGraph {
  ...
  for (let i = 0; i < config.steps.length; i++) {
    ...
    if ("type" in step && step.type === "condition") {
      if (!steps.has(step.on_true))
        throw new Error(`Condition step '${step.id}' references undefined step '${step.on_true}'`);
      if (!steps.has(step.on_false))
        throw new Error(`Condition step '${step.id}' references undefined step '${step.on_false}'`);
