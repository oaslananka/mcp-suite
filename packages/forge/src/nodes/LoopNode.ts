...
export class LoopNode extends BaseNode {
  ...
  async execute(stepConfig: StepConfig, ctx: RunContext): Promise<StepResult> {
    ...
    try {
      const items = this.transformer.transform(
        stepConfig.over as string,
        ctx.dataBus.toTemplateContext()
      );
      if (!Array.isArray(items)) {
        return {
          status: "failed",
          error: `Loop 'over' expression did not evaluate to an array: ${stepConfig.over}`,
        };
      }
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        for (const childConfig of stepConfig.steps as StepConfig[]) {
          ...
          if (result.status === "failed") {
            throw new Error(
              `Loop step ${childConfig.id} failed on iteration ${i}: ${result.error}`
            );
          }
