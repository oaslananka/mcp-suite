...
export class GraphQLParser {
  ...
  parseIntrospection(
    introspectionResult: IntrospectionQuery | { data: IntrospectionQuery }
  ): ParsedGraphQLSchema {
    // Build client schema from full introspection query result
    const schema = buildClientSchema(
      hasIntrospectionData(introspectionResult) ? introspectionResult.data : introspectionResult
    );
  ...
  ): Array<GraphQLField<unknown, unknown>> {
