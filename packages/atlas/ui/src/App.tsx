...
function HomePage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    void fetchJson<{ items: AtlasServer[] }>("/api/trending").then((data) =>
    ...
    <>
      <section className="hero">
        <div>
          ...
          <p className="lede">
            Atlas brings discovery, lightweight quality scoring, and install guidance into one
            focused registry experience for the MCP ecosystem.
          </p>
        </div>
      ...
      <section className="catalog-grid">
        ...
        {!loading && servers.length === 0 ? (
          <div className="empty-state">
            ...
            <p>
              Try removing a filter or search for a broader capability like "database" or "browser".
            </p>
          </div>
        ) : null}
      </section>
      ...
      <form className="detail-card form-card" onSubmit={(event) => void submitForm(event)}>
        ...
        <label>
          ...
          <input
            ...
            onChange={(event) =>
              setFormState((current) => ({ ...current, packageName: event.target.value }))
            }
            required
          />
        </label>
        ...
        <label>
          ...
          <input
            ...
            onChange={(event) =>
              setFormState((current) => ({ ...current, homepage: event.target.value }))
            }
