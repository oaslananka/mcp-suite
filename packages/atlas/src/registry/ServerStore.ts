...
export class ServerStore {
  ...
  add(
    server: Omit<MCPServerRecord, "id" | "createdAt" | "updatedAt" | "qualityScore">
  ): MCPServerRecord {
  ...
  listTags(): string[] {
    ...
    >;
    return [
      ...new Set(rows.flatMap((row) => JSON.parse(String(row["tags_json"])) as string[])),
  ...
  private toRecord(row: Record<string, unknown>): MCPServerRecord {
    const configSchema =
      typeof row["config_schema_json"] === "string"
