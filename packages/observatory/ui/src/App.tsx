...
export function App(): JSX.Element {
  ...
  useEffect(() => {
    ...
    void fetchJson<{ items: MetricPoint[] }>("/api/metrics?name=latency&minutes=60").then((data) =>
      setMetrics(data.items)
    );
    void fetchJson<{ items: TraceRecord[] }>("/api/traces?limit=25").then((data) =>
      setTraces(data.items)
    );
    void fetchJson<{ items: AlertLike[] }>("/api/anomalies").then((data) =>
    ...
    <main className="shell">
      <section className="hero">
        <div>
          ...
          <p className="lede">
            Observatory packages operational insight for MCP workloads with dashboard summaries,
            trace views, anomaly detection, and alert history.
          </p>
        </div>
        <nav className="tab-row">
          ...
          <button
            type="button"
            className={path === "/traces" ? "tab active" : "tab"}
          ...
          <button
            type="button"
            className={path === "/anomalies" ? "tab active" : "tab"}
      ...
      {path === "/" ? (
        <Dashboard
          summary={summary}
          latencySeries={metrics.map((metric) => ({
