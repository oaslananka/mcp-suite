...
describe("Observatory instrumentation helpers", () => {
  it("collects spans and records client/server metrics", () => {
    ...
    expect(store.queryMetrics("calls", new Date(Date.now() - 3600_000), new Date())).toHaveLength(
      1
    );
    expect(store.queryMetrics("errors", new Date(Date.now() - 3600_000), new Date())).toHaveLength(
      1
    );
    expect(
      store.queryMetrics("latency", new Date(Date.now() - 3600_000), new Date()).length
