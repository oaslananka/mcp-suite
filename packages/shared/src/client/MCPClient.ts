...
export class MCPClient extends EventEmitter {
  ...
  private pendingRequests: Map<
    string | number,
    { resolve: (res: unknown) => void; reject: (err: unknown) => void; timeout?: NodeJS.Timeout }
  > = new Map();
  ...
  public async connect(): Promise<InitializeResult> {
    ...
    await this.withTimeout(
      ...
      "Timed out while opening transport"
    );
  ...
  public async request<T = unknown>(method: string, params?: unknown): Promise<T> {
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(
          new Error(`Request timed out for method "${method}" after ${this.requestTimeoutMs}ms`)
        );
